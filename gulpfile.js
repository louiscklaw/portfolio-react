...
let asset_file_list = execSync('find src/assets -type f',{encoding: 'utf-8'})

const {resize} = require('easyimage');

const file_filter_list = ['','.gitkeep'];

function filterOutEmptyFilename(filename_in){
  return filename_in != ''
}

function filterOutGitKeepFile(filename_in){
  return !(filename_in.search('.gitkeep') > -1)
}

function tidyUpFileList(file_list_in){
    return file_list_in
      .split(/\s/)
      .filter( x => filterOutEmptyFilename(x) )
      .filter( x => filterOutGitKeepFile(x) )
      .sort()
  }

  function getCheckSum(file_list_in){
    return md5(file_list_in)
  }

function js() {
    return src( 'src/js/sw.js/*.js', {
            sourcemaps: true
        } )
        .pipe(order([
            'checksums.js',
...
function getFBRefPath(file_list_in){
  // convert to firebase path, remove "src"
  return file_list_in
    .map(x => x.replace('src/', ''))
}

let static_file_list_template = `
// static_file_list.js
const static_file_list = ${JSON.stringify(
  getFBRefPath(
  tidyUpFileList( asset_file_list )
  ) )
  }
`

let checksums_template = `
// checksums.js
...
function RootNpmInstall(done){
  // console.log(execSync('pwd',{encoding: 'utf-8'}))
  execSync('npm install')
  done()
}

function srcNpmInstall(done){
  execSync('cd src && npm install')
  done()
}
...
exports.helloworld = parallel(RootNpmInstall, srcNpmInstall)
